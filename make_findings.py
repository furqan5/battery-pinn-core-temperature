...
that package is the **Simulink electro-thermal model**, not experimental data — `run_model.m`
...
IEEE Trans. Sustainable Energy 6(4):1190–1199 — two HEV drive cycles on a 26650 A123 LFP cell
...
|---|---|---|---|
| P1 | core−surface max between 0.5 and 5 K | **MISS** | **7.13 K** (DS1), 6.55 K (DS2) — 43 % above the interval |
...
| P6 | core error ≥ 2× surface-fit error | **{P6_0}** | ratio {s0['ratio']:.2f} (order 0), {s1['ratio']:.2f} (order 1) |

Three hits, two or three misses. The misses are the useful part:

- **P1** underestimated the gradient. The cell runs at 8 °C ambient with 30 A peaks (~13 C), so
  Bi ≈ 1.23 and the gradient is far larger than budgeted. More signal than expected.
- **P3** was wrong in an instructive direction — see §4, where the CRLB blessed a parameter set
  that then failed catastrophically.
- **P6** {'held' if P6_0 == 'HIT' else 'did not hold'}: the core error is
  {s0['ratio']:.2f}× the surface error, {'above' if s0['ratio'] >= 2 else 'below'} the predicted
  2× floor.
...
|---|---|---|---|---|---|---|---|
| order 0 (scalar R_eff) | {s0['R']:.3f} mΩ | {s0['surf']:.4f} K | **{s0['core']:.4f} K** | {s0['cmax']:.4f} K | {s0['bias']:+.4f} K | {s0['pct_max']:.2f} % | {s0['ratio']:.2f} |
| order 1 (R_eff(1+a₁x)) | {s1['R']:.3f} mΩ | {s1['surf']:.4f} K | **{s1['core']:.4f} K** | {s1['cmax']:.4f} K | {s1['bias']:+.4f} K | {s1['pct_max']:.2f} % | {s1['ratio']:.2f} |

Core-minus-surface *gradient* RMSE: **{s0['gradrmse']:.4f} K** on a gradient whose rms is
{np.sqrt((grad**2).mean()):.4f} K — i.e. the reconstruction recovers
...
collocation set — never on proximity to the core.

Recovered **R_eff = {s0['R']:.3f} mΩ** sits against an entirely independent anchor: regressing the
measured V on I gives {1000*rec.R_ohmic_reg:.3f} mΩ of ohmic slope for this record. Agreement to
...
parameters** — surface only, h and k from DS1's surface-only fit, scalar R_eff, fitted on DS2:

| | R_eff | surface RMSE | core RMSE |
|---|---|---|---|
| classical FV + 1 scalar | {1000*cl_R:.3f} mΩ | {cl_surf:.4f} K | **{cl_core:.4f} K** |
| inverse PINN (order 0) | {s0['R']:.3f} mΩ | {s0['surf']:.4f} K | **{s0['core']:.4f} K** |
...
classical solver would need re-deriving for — not that it is more accurate.
...
surface fit is the best of the whole table and the core error is nearly 2× worse.
...
headline number depends on an input, not an output. Our leak-free k = {K_FIXED:.5f} — obtained
from a surface-only fit on the *other* record — happens to land within ~1 % of the core-optimal
...
2. **The reconstruction captures the gradient, not just the trajectory.** Predicted
   core−surface tracks measured core−surface to {s0['gradrmse']:.3f} K rms. This is the part that
   carries spatial information, and it is the part previous notebooks could never check.
3. **The Bi/2 plausibility gate holds.** Predicted (core−surf)/(surface rise) = 0.6196 against
...
2. **R_eff is a blend, not an internal resistance.** The entropic term is off in the headline
   result, so it is labelled R_eff throughout. Quantifying that choice: mean(|Q_rev|) is 57–63 %
   of mean ohmic, but the **signed** mean is only −1.4 %, because the term alternates with
   charge and discharge and largely cancels. Switching it on (Forgez et al. 2010, −0.5 mV/K at
   50 % SOC) moves R_eff by 1.1 % and the core RMSE from 0.8895 K to 0.8927 K — **0.4 %**. So
   the blend is real but, for the thermal prediction on this duty cycle, immaterial.
3. **No shape R(x) has been recovered.** The DoD window spans only 11–14 % of the axis. Order-1
...
1. **Report the parameter correlation matrix, not just marginal standard deviations.** The CRLB
   rated {{h, k, ρc_p}} identifiable at 3.9 % worst rel sd; the actual fit then drove k 1140 %
   away, ~300× outside the CRLB ellipse, producing 4 K of core error. The bound assumes a correct
   model and white noise. The 0.99 correlation between k and ρc_p was the statistic that warned.
...
3. **Fourier time features can hurt badly.** On a controlled problem with a known answer, error
   ...
   bandwidth anyway (n_freq ≈ 1500) and degraded the optimisation landscape.
...
5. **Test the discretisation against closed-form solutions at machine precision.** Two silent
   bugs — a boundary source broadcast to every cell, and an O(dr/4R) wall-conductance bias — were
...
1. **Use both records jointly with a shared k.** DS1 and DS2 have different current statistics
   ...
   k–h collinearity that defeats a single record. Cheap — no new data needed.
